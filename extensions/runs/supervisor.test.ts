/**
 * Tests for the per-run anomaly polling loop (ADR 014, M3-3; amended 2026-05-22).
 *
 * Two pause-tier rules: `unbounded_diff`, `heartbeat_missed`.
...
describe("pauseRun", () => {
	...
	it("SIGTERMs the PID, marks the run paused, emits anomaly_detected + run_finished", async () => {
		...
		expect(anomaly.rule).toBe("unbounded_diff");
		expect(anomaly.runId).toBe("r1");
		expect(anomaly.details.filesChanged).toBe(60);
		...
		expect(finished.details.paused_reason).toBe("unbounded_diff");
...
/* ============================== STATUS_RANK race regression (issue #13) ============================== */

describe("pauseRun + STATUS_RANK race regression", () => {
	beforeEach(() => {
		vol.reset();
	});

	it("does not overwrite a more-terminal disk status (paused stays paused over cancelled)", async () => {
		// Setup: supervisor's pauseRun fires and writes `paused`. Now simulate
		// the runner's close-handler arriving with `cancelled` (the bug shape).
		// We test pauseRun's gate directly: a second pauseRun call against a
		// summary that's already paused on disk must not churn or downgrade.
		const summary: BackgroundRunSummary = {
			runId: "r-race",
			questId: "q1",
			workItemId: "001",
			...
			stderrPath: "/y",
			reportPath: "/z",
			statusPath: "/project/.pi/quests/q1/runs/r-race.json",
			worktreePath: "/project/.pi/quests/q1/worktrees/r-race",
		};
		vol.mkdirSync("/project/.pi/quests/q1/runs", { recursive: true });
		// Pre-seed: another writer already promoted to `paused` on disk.
		vol.writeFileSync(
			"/project/.pi/quests/q1/runs/r-race.json",
			JSON.stringify({ ...summary, status: "paused", paused_reason: "heartbeat_missed" }),
		);

		const killSpy = vi.spyOn(process, "kill").mockImplementation(() => true);
		// pauseRun called with the in-memory `running` summary; the disk says
		// `paused`; the gate should keep `paused` (no downgrade, no re-emit).
		await pauseRun({
			cwd: "/project",
			summary,
			rule: "unbounded_diff",
		...
		const diskAfter = JSON.parse(
			vol.readFileSync("/project/.pi/quests/q1/runs/r-race.json", "utf-8") as string,
		) as BackgroundRunSummary;
		expect(diskAfter.status).toBe("paused");
		expect(diskAfter.paused_reason).toBe("heartbeat_missed");

		// No new events emitted.
		const eventsPath = "/project/.pi/quests/q1/telemetry/events.jsonl";
		const fileExists = vol.existsSync(eventsPath);
		if (fileExists) {
			const jsonl = vol.readFileSync(eventsPath, "utf-8") as string;
